...
-- =============================================================================
-- DOMINIO 9: HÁBITOS, GAMIFICACIÓN & TRIBUS (XP & STREAKS)
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS habits (
    ...
    title VARCHAR(255) NOT NULL,
    type VARCHAR(50) DEFAULT 'BUILD' NOT NULL, -- 'BUILD' | 'BREAK'
    category VARCHAR(100) DEFAULT 'lifestyle',
    input_type VARCHAR(50) DEFAULT 'BOOLEAN', -- 'BOOLEAN' | 'NUMERIC'
    unit VARCHAR(50),
...
CREATE TABLE IF NOT EXISTS habit_logs (
    ...
    value NUMERIC(10, 2),
    zone VARCHAR(50) DEFAULT 'NONE', -- 'NONE' | 'LOW' | 'HIGH'
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL,
...
CREATE TABLE IF NOT EXISTS athlete_wallets (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE TABLE IF NOT EXISTS wallet_transactions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    wallet_id UUID NOT NULL REFERENCES athlete_wallets(id) ON DELETE CASCADE,
    amount INTEGER NOT NULL,
    transaction_type VARCHAR(50) NOT NULL, -- 'EARNED' | 'SPENT' | 'REFUNDED'
...
CREATE TABLE IF NOT EXISTS squad_members (
    ...
    CONSTRAINT uq_squad_member UNIQUE (squad_id, client_id)
);
...
CREATE TABLE IF NOT EXISTS athlete_challenges (
    ...
    title VARCHAR(255) NOT NULL,
    type VARCHAR(50) DEFAULT 'STREAK', -- 'STREAK' | 'VOLUME' | 'CONSISTENCY'
    target_value INTEGER DEFAULT 7 NOT NULL,
...
CREATE TABLE IF NOT EXISTS challenge_progress_events (
    ...
    source VARCHAR(100) DEFAULT 'habit_check_in',
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL
);

-- =============================================================================
-- DOMINIO 10: FINANZAS DEL COACH & PLANES COMERCIALES
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS commercial_plans (
    ...
    duration_text VARCHAR(100) DEFAULT 'Mensual',
...
CREATE TABLE IF NOT EXISTS client_memberships (
    ...
    enrolled_date TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL
);
...
-- =============================================================================
-- DOMINIO 11: TEMPLATES MAESTROS, BIOMECÁNICA & READINESS
-- =============================================================================

CREATE TABLE IF NOT EXISTS master_templates (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    trainer_id UUID REFERENCES professionals(id) ON DELETE SET NULL,
    title VARCHAR(255) NOT NULL,
    description TEXT,
    structure JSONB DEFAULT '{}' NOT NULL,
    tags TEXT[] DEFAULT '{}',
...
CREATE TABLE IF NOT EXISTS exercise_targets (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    superset_group_id UUID NOT NULL REFERENCES superset_groups(id) ON DELETE CASCADE,
    exercise_id UUID REFERENCES exercises(id) ON DELETE SET NULL,
    custom_exercise_name VARCHAR(255),
    order_index INTEGER DEFAULT 0 NOT NULL,
    sets INTEGER DEFAULT 3 NOT NULL,
    reps VARCHAR(50) DEFAULT '10-12',
    rpe VARCHAR(50) DEFAULT '8',
...
CREATE TABLE IF NOT EXISTS daily_readiness (
    ...
    notes TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL,
    CONSTRAINT uq_client_readiness_date UNIQUE (client_id, date)
);
...
CREATE POLICY tenant_isolation_commercial_plans ON commercial_plans
    FOR ALL
    USING (tenant_id = NULLIF(current_setting('app.current_tenant_id', true), '')::uuid);

CREATE POLICY tenant_isolation_client_memberships ON client_memberships
    FOR ALL
    USING (tenant_id = NULLIF(current_setting('app.current_tenant_id', true), '')::uuid);

CREATE POLICY tenant_isolation_master_templates ON master_templates
    FOR ALL
    USING (tenant_id = NULLIF(current_setting('app.current_tenant_id', true), '')::uuid);

CREATE POLICY tenant_isolation_video_reviews ON video_reviews
    FOR ALL
    USING (tenant_id = NULLIF(current_setting('app.current_tenant_id', true), '')::uuid);
